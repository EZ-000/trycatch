...
public class AcceptAnswerResponseDto implements Serializable {
    ...
    public static AcceptAnswerResponseDto from(
        ...
        return AcceptAnswerResponseDto.builder().questionId(question.getId()).author(author).category(question.getCategoryName()).title(question.getTitle()).content(question.getContent()).errorCode(question.getErrorCode()).tags(List.of(question.getTags().split(","))).likeCount(question.getLikes()).answerCount(answerDtoList.size()).viewCount(question.getViewCount()).timestamp(timestamp).updatedAt(question.getUpdatedAt().toEpochMilli()).isLiked(isLiked).isSolved(question.getChosen()).isBookmarked(isBookmarked).answers(answerDtoList).build();
    }
}
