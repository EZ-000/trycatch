...
public class FindFeedResponseDto {
    ...
    public static FindFeedResponseDto newDummy(Integer amount) {
        final List<Feed> feeds = LongStream.range(1, amount + 1).mapToObj(Feed::newDummy).collect(
                Collectors.toList());
    ...
    public static class Feed {
        ...
        public static Feed newDummy(Long id) {
            ...
            return Feed.builder().feedId("" + id).title("title-" + id).summary("summary-" + id).companyName(
                    "company-" + id).createdAt("2023-01-01").url("https://i8e108.p.ssafy.io/").thumbnailImage(
                    "https://i8e108.p.ssafy.io/assets/favicon-1170e8b7.ico").tags(tags).build();
        }
